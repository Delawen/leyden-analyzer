...
@Command(name = "warning", mixinStandardHelpOptions = true, version = "1.0", description = {
        "Help detect and clarify warnings found. By default, it lists incidents detected on the logs." }, subcommands = {
...
class WarningCommand extends BaseCommand {
    ...
    @CommandLine.Option(names = { "-t",
            "--type" }, arity = "0..*", split = ",", paramLabel = "<type>", description = "Restrict the command to this type of warnings", completionCandidates = Types.class)
    protected String[] types;

    @CommandLine.Option(names = { "-l",
            "--limit" }, arity = "0..1", paramLabel = "<limit>", description = "Limit number of incidents per type listed")
    protected Integer limit;

    @CommandLine.Option(names = { "--identifier", "-i" }, description = {
            "The object identifier. Use the full qualified name." }, defaultValue = "", arity = "0..1", paramLabel = "<id>", completionCandidates = Identifiers.class)
    ...
    @Command(mixinStandardHelpOptions = true, version = "1.0", subcommands = { CommandLine.HelpCommand.class }, description = {
            "Mark a warning as safe, clearing it from the list of warnings.",
            "You can't recover it afterwards without doing a new check or loading the log file." })
    public void rm(@CommandLine.Parameters(paramLabel = "<id>", description = "warning to clear") String id) {
    ...
    @Command(mixinStandardHelpOptions = true, version = "1.0", subcommands = { CommandLine.HelpCommand.class }, description = {
            "Automatically try to detect inconsistencies and potential defects in your run.",
